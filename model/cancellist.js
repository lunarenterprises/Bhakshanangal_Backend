...
module.exports.CheckList = async (user_id, page, limit) => {
    var Query = `select order_product_order_id as order_id,order_product_product_id as product_id ,order_product_quantity as order_quantity,order_product_unit as order_unit,created_at,modified_at,order_status,order_payment_method,gift_card,delivery_mode_status from  bh_order_product
    ...
    left join bh_delivery_mode on  delivery_status = delivery_mode_id where order_status = 'cancelled' order by order_id desc limit ${limit} offset ${page}`;
    var data = query(Query, [user_id]);
    return data;
};
